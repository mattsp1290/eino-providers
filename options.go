...
type Options struct {
	...
	// SessionID supplies the configured fallback OpenCode Go conversation
	// identifier. Each request needs a valid configured or context session; a
	// session attached to an operation context takes precedence. It is ignored by
	// every other backend.
	SessionID string
	...
	// HTTPClient supplies transport customization. OpenCode Go treats it as a
	// trusted, unauthenticated base client that opencode-auth-go copies and decorates.
